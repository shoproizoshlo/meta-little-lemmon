...
export const SetDataProvider = (props) => {
  ...
  const HandleUpdateProfile = (updatedProfile, triggerFunc) => {
    axios
      .put(
        `https://little-lemon-restaurant-database.onrender.com/login-profiles/${id}`,
        updatedProfile
      )
      .then((response) => {
        triggerFunc();
  ...
  const HandleRemoveItemFormCartOrOrder = (idForItem, Option) => {
    ...
    const grabProfileType = grabProfile[Option].filter(
      (item) => item["id"] !== idForItem
    );
  ...
  const HandleRemoveEmptyDateFormReservations = () => {
    if (reservations.bookings !== undefined) {
      const updatedBookings = reservations["bookings"].filter(
        (element) => element["timeSlots"].length !== 0
      );
